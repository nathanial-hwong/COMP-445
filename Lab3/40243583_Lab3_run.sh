...
run_one() {
  ...
  ip netns exec "$SERVER_NS" bash -lc "cd '$LAB_DIR' && $PYTHON_BIN 40243583_Lab3_server.py --bind $SERVER_IP --port $PORT --segment-size $SEGMENT_SIZE --timeout $TIMEOUT --base-dir . --trace-file '$server_log_tmp'" &
  local server_pid=$!
  ...
  set +e
  ip netns exec "$CLIENT_NS" bash -lc "cd '$LAB_DIR' && $PYTHON_BIN 40243583_Lab3_client.py $SERVER_IP $PORT '$INPUT_FILE' --segment-size $SEGMENT_SIZE --timeout $TIMEOUT --output '$output_file' --trace-file '$client_log_tmp'"
  local client_rc=$?
  set -e
